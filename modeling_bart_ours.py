...
class PageSum(BartPretrainedModel):
    def __init__(self, config: BartConfig):
        super().__init__(config)
    ...
    def forward(
        ...
        if encoder_outputs is None:
            ...
            encoder_outputs["last_hidden_state"] = encoder_outputs[0].view(batch_size, -1, encoder_outputs[0].size(-1))  # change the shape back
        # If the user passed a tuple for encoder_outputs, we wrap it in a BaseModelOutput when return_dict=True
        elif return_dict and not isinstance(encoder_outputs, BaseModelOutput):
            encoder_outputs = BaseModelOutput(
                last_hidden_state=encoder_outputs[0],
...
class PageSumModel(BartPretrainedModel):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [r"final_logits_bias", r"lm_head\.weight", r"mlp\.weight"]

    def __init__(self, config: BartConfig, num_layers=1):
        super().__init__(config)
        self.model = PageSum(config)
        self.register_buffer("final_logits_bias", torch.zeros((1, self.model.shared.num_embeddings)))
        self.lm_head = nn.Linear(config.d_model, self.model.shared.num_embeddings, bias=False)
        if num_layers > 0:
            encoder_layer = nn.TransformerEncoderLayer(d_model=config.d_model, nhead=8)
            self.att = nn.TransformerEncoder(encoder_layer, num_layers=num_layers)
    ...
    def attention_score(
        ...
        if self.num_layers > 0:
            ...
            s = self.lin(lm_logits)
        s = torch.softmax(s, dim=1)
        s = s.view(batch_size, seq_len, seq_num)
        return s
...
class PageSumModelBase(BartPretrainedModel):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [r"final_logits_bias", r"lm_head\.weight", r"mlp\.weight"]
